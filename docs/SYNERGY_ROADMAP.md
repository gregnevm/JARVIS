...
### SY-6 — Лічильник як декоратор — `MeterLLM × usage-паспорти × білінг Стовпа A` — 🔴 todo
...
- [ ] **SY-6.1** `MeterLLM` у `jarvis_core/llm/decorators.py` (за прапором; батч-ingest, щоб не
      душити стор по одному паспорту на виклик)
- [ ] **SY-6.2** `/v1` usage-ендпоінт читає агрегати зі стору (узгодити з `saas/usage.py`,
      AP-трек — один SSOT, не два лічильники)
- [ ] **SY-6.3** kaizen O3 ECO: self-score рахується з `kind:usage` (замість ручних оцінок)
- [ ] **SY-6.4** /platform: cost-дашборд = tag-запит по `kind:usage`
