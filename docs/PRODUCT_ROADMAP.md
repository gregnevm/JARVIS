...
- [ ] `health_watch` — проактивні алерти в Telegram
- [ ] `remote` — `/file`, `/macro`, `/tasks`, `/see`
- [ ] `jobs` + `macros` — cron-макроси без LLM
- [ ] `jarvis_core` facade + pipeline
- [ ] SD Forge / `image_gen` (локальна генерація)
- [ ] `browser.py` — каркас Playwright (C3)
...
| 0.1.2 | Єдиний autostart: `scripts/autostart.ps1` + `verify_stack.ps1` | Ребут → smoke < 5 хв | [ ] |
| 0.1.3 | Пройти [`ENV_CHECKLIST.md`](ENV_CHECKLIST.md); `WEBAPP_DEV_OPEN=false` у проді | Немає витоку токенів у логах | [ ] |
| 0.1.4 | Бекапи: `data/`, postgres volume, `data/twin/`, `computer.jsonl` | Runbook відновлення | [ ] |

### 0.2 Закрити WIP

| # | Задача | Статус |
|---|--------|--------|
| 0.2.1 | Стабілізувати `health_watch` + тести | [ ] |
| 0.2.2 | Стабілізувати `remote` + `macros` + `jobs` | [ ] |
| 0.2.3 | `agent_turn` — єдиний turn-path, паритет stream/deliver | [ ] |
| 0.2.4 | CI green на нових модулях | [ ] |

### 0.3 Smoke як продукт

| # | Задача | Статус |
|---|--------|--------|
| 0.3.1 | Розширити [`SMOKE_TEST.md`](SMOKE_TEST.md) до щоденного чеклиста (15+ пунктів) | [ ] |
...
| # | Задача | Статус |
|---|--------|--------|
| 1.1.1 | Персональний профіль у промпті / memory (стиль, мова, табу) | [ ] |
| 1.1.2 | Summarization довгих тредів перед RAG (DESIGN §4.8) | [ ] |
| 1.1.3 | Стабільний шлях файлів/фото → `parse_file` / `ocr_image` у hybrid | [ ] |
| 1.1.4 | Голос E2E: STT → agent → TTS за замовч.; toggle у Mini App | [ ] |

### 1.2 Mini App — центр керування

| # | Задача | Статус |
|---|--------|--------|
| 1.2.1 | Статус: Ollama, моделі, host-agent, Docker, останні помилки | [ ] |
| 1.2.2 | Перемикачі: `AGENT_MODE`, streaming, voice reply | [ ] |
| 1.2.3 | Read-only: останні N сесій з postgres | [ ] |
...
| # | Задача | Статус |
|---|--------|--------|
| 1.3.1 | Покращений `web_fetch` (таймаути, readability) | [ ] |
| 1.3.2 | Експорт нагадувань / ICS (опц.) | [ ] |
...
| # | Задача | Статус |
|---|--------|--------|
| 1.4.1 | SD Forge: `setup_sd_forge.ps1`, `start_sd_forge.ps1`, `IMAGE_GEN_URL` | [ ] |
| 1.4.2 | Horde/pollinations — лише явний opt-in | [ ] |
| 1.4.3 | VRAM policy: image gen не вбиває agent model (черга / «зайнято») | [ ] |
...
|------|--------|-----|--------|
| C3.1 | Playwright у tools + `browser_*` у toolkit | Інструменти в agent loop | [ ] |
| C3.2 | ADR: Chrome profile (чистий vs logged-in) | Документовано | [ ] |
| C3.3 | Confirm для mutating browser actions | Inline ✅/❌ | [ ] |
| C3.4 | Smoke: URL → заголовок/текст у Telegram | E2E pass | [ ] |

### 2.2 C4 — UI Automation (T3)

| Крок | Задача | Статус |
|------|--------|--------|
| C4.1 | host-agent: `/uia/*`, `/window/*` | [ ] |
| C4.2 | Toolkit: `uia_invoke`, `window_focus` | [ ] |
| C4.3 | UX для `computer_learned` (довірені cmdlet/exe) | [ ] |

### 2.3 C5 — Admin (обмежено)

| Крок | Задача | Статус |
|------|--------|--------|
| C5.1 | `COMPUTER_ALLOW_ADMIN` лише `ADMIN_USER_IDS` | [ ] |
| C5.2 | Подвійне підтвердження + audit tier `admin` | [ ] |
| C5.3 | Runbook відкату шкоди | [ ] |

### 2.4 C6 — Vision (T4) — опційно

| Крок | Задача | Статус |
|------|--------|--------|
| C6.1 | On-demand vision model / unload 7B | [ ] лише якщо VRAM дозволяє |
| C6.2 | `screen_click` з координат — останній резерв | [ ] |
...
| 2.5.2 | Макроси без LLM — safe path для рутини | [ ] |
| 2.5.3 | Rate limit computer actions / годину | [ ] |
| 2.5.4 | Mini App: tail `computer.jsonl` | [ ] |
...
|---|-----------|--------|------|
| A.1 | ModelRegistry (`twin/app/registry.py`) | [x] є | [ ] promote/rollback UI |
| A.2 | SessionLogger JSONL | [x] є | [ ] авто-ingest кожної сесії |
| A.3 | Sync API | [x] є | [ ] експорт для датасету |
| A.4 | n8n → legacy, не default | [ ] | [ ] документ міграції |

### 3.2 Етап C — Дані (critical path)

| Крок | Задача | Обсяг | Статус |
|------|--------|-------|--------|
| C.1 | SessionLogger → курація найкращих діалогів | ручний + фільтри | [ ] |
| C.2 | ShareGPT JSONL | 500 train / 50 holdout | [ ] |
| C.3 | Eval harness (format + LLM-as-judge) | локально | [ ] |
| C.4 | Correctness gate перед promote LoRA | smoke + eval ↑ | [ ] |

### 3.3 Етап D — Training (cloud-burst, ADR-007)

| Крок | Задача | Статус |
|------|--------|--------|
| D.1 | `training/` — RunPod + Unsloth скрипт | [ ] |
| D.2 | LoRA → registry → Blue-Green symlink | [ ] |
| D.3 | Ollama Modelfile / adapter load | [ ] |
| D.4 | Scheduler: retrain кожні +200 якісних прикладів | [ ] |

### 3.4 Архітектура коду

| # | Задача | Статус |
|---|--------|--------|
| 3.4.1 | `jarvis_core` — gateway/tools = транспорт, логіка в facade | [ ] |
| 3.4.2 | `OllamaAdapter` + майбутній `KoboldAdapter` | [ ] |
| 3.4.3 | Cascade routing (classify → chat/agent/computer) | [ ] |
...
|---|--------|--------|--------|
| 5.1 | Тести | Contract host-agent ↔ tools; golden traces | [ ] |
| 5.2 | Логи | structlog JSON; `request_id` gateway→tools | [ ] |
| 5.3 | Метрики | tok/s, tool latency, RAG hit → Mini App | [ ] |
| 5.4 | S1 черга | Redis queue + workers — лише при навантаженні | [ ] |
| 5.5 | Threat model | computer, web_fetch, uploads | [ ] |
...
| # | Артефакт | Статус |
|---|----------|--------|
| 6.1 | `Install-JARVIS.ps1` (Docker, Ollama, models, host-agent) | [ ] |
