...
**Останній прохід:** 2026-07-27 (kaizen window-6 iter1)
...
### Прохід 2026-07-27 (kaizen window-6, iter1) — клас дефекту «SELECT дрейфнув від спільного мапера»
- **Знайдено logic-bug** (ultracode judge-panel, 4 скаути + 2 судді; обидва судді відтворили його
  незалежно офлайн): `DB.context_ledger` (`memory/app/db.py`) селектив у `recent`-підзапиті лише
  `id, kind, source, summary, tags, created_at`, але кожен рядок мапиться **спільним** fail-fast
  мапером `DB._context_json`, який читає ще `r["sensitivity"]`, `r["ref"]`, `r["event_ts"]`.
  Результат: `KeyError` на **будь-якому непорожньому** журналі → 500 з `/context/ledger`.
- **Чому симптом брехав про причину:** проксі `gateway/app/client_api/context.py` ловить
  `ServiceError` і віддає `{"total": 0, …, "error": "memory_unreachable"}`. Тобто E3-журнал
  прозорості (єдиний екран, де користувач бачить, що про нього зібрано; споживач —
  `SettingsActivity` мобільного клієнта) виглядав як «memory лежить», хоча memory була жива.
- **Чому тести не ловили:** `memory/tests/test_context_routes.py` мокає `context_ledger`
  `AsyncMock`-ом, `gateway/tests/test_client_api_context.py` — фікстурою. Жоден тест не проганяв
  реальний SQL через реальний мапер, тож дрейф SELECT-а був невидимий для всього гейта.
- **Виправлення:** SELECT ledger-а розширено до повної проєкції паспорта (той самий набір, що вже
  в `search_context`/`recent_context`). Мапер **свідомо лишено fail-fast** — жодних `r.get`:
  інакше наступний дрейф став би тихим (порожні поля замість помилки).
- **Регресійний лок (головна цінність):** `memory/tests/test_context_projection.py` — fake-pool
  парсить список колонок із самого SQL і віддає рядок **рівно з цими** ключами (семантика
  `asyncpg.Record`), тому тест лочить **інваріант** «SELECT ⊇ поля, які читає мапер», а не текст
  запиту. Параметризовано по `search_context`/`recent_context`/`context_ledger`; `pending_context`
  покрито окремо (власний інлайн-мапер). Будь-який майбутній read-шлях, що розійдеться з мапером,
  тепер падає локально, а не 500-ить у проді.
- **Верифікація:** `pytest memory/tests` ✅ 103 (96 до зміни + 7 нових); `mypy memory/app` ✅ 11
  файлів; `ruff check` ✅; `pytest gateway/tests/test_client_api_context.py` ✅ 21 (проксі
  незмінний — зміна адитивна). До фіксу нові тести падали саме `KeyError: 'sensitivity'`.
- **Урок:** коли N запитів мапляться ОДНИМ спільним мапером, мапер стає неявним контрактом на
  набір колонок — і жоден тип не тримає цей контракт (`asyncpg.Record` динамічний). Або лочити
  його тестом, або тримати список колонок в одному місці; мокати сам метод — значить не тестувати
  контракт узагалі.
...
- **Знайдено активний баг втрати даних** (live-reproduced): `_parse_inline_tool_calls`
  (`tools/app/agent.py`) — фолбек, що ловить tool call, який модель (qwen2.5 в Ollama)
  віддала текстом `<tool_call>{...}</tool_call>` замість поля `tool_calls`. Стара
  регулярка `(\{.*?\})` (нежадібна) обрізала вкладений `arguments` на **першій
  внутрішній `}`** → `json.loads` падав у `JSONDecodeError` → `args` тихо ставали `{}`.
  Тобто `mcp_call`/`code_edit_batch`/будь-який виклик із вкладеними аргументами
  виконувався **без аргументів** — мовчазна втрата даних на шляху, від якого залежить
  весь агент-луп qwen2.5.
- **Фікс:** регулярка тепер матчить лише ВІДКРИТТЯ об'єкта, а сам об'єкт дочитує новий
  чистий балансувальник дужок `_scan_balanced_json` (string-aware: дужки в рядкових
  значеннях `"a{b}c"` не ламають баланс; ітеративний — без рекурсії в скануванні).
- **Адверсарний multi-agent рев'ю (ultracode) виявив і ми закрили 2 реальні наслідки
  нового коду:** (1) **HIGH** — квадратична складність на повторюваних обрізаних
  відкриттях (кожне ре-сканувало до EOF, O(n²); 50 KB → ~9 с блокування event-loop):
  фікс — `break` коли об'єкт незбалансований (решта буфера — один незакритий об'єкт,
  пізніших top-level викликів бути не може); парс став строго лінійним (≈38 с → 0.09 с
  на бенчмарку). (2) **MED** — `json.loads` кидає `RecursionError` (НЕ підклас
  `JSONDecodeError`) на глибокій вкладеності → виняток втікав, порушуючи контракт
  «фолбек ніколи не падає»: розширено `except (json.JSONDecodeError, RecursionError)`.
  Свідомо НЕ додавали обрізку входу `[:N]` — надто тісний кеп повертає той самий клас
  тихої втрати даних для легітимних великих викликів.
- **Тести:** +9 (`tools/tests/test_agent.py`): вкладений об'єкт, список+вклад,
  дужки-в-рядку, кілька викликів (один вкладений), обірваний об'єкт (no-hang),
  невалідний-але-збалансований JSON, глибока вкладеність (no-raise), лінійність на
  20 000 обрізаних відкриттях, багаторядковий pretty-print. Фінальна верифікація
  (окремий агент): 300 000 fuzz-входів, 0 порушень інваріанту.
- **Верифікація:** `pytest tools/tests/test_agent.py` ✅ 36/36; `mypy tools/app`
  ✅ 91 файл (strict). Лише allow-paths (`tools/**`), без секретів/deny-paths.
- **Урок:** коли «зрізаний» парсер заміняєш на повний, повнота відкриває нові входи
  (глибока вкладеність, повтори) — те, що раніше тихо відкидалось, тепер доходить до
  `json.loads`/циклу. Розширення прийому даних треба завжди супроводжувати ре-аналізом
  складності й контракту «не падати».
