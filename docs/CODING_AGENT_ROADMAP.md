...
| Self-review | **7/10** | `code_review` self-pass (CA-5.1) + Coder→Reviewer→Tester pipeline (CA-5.2); бракує авто-fix перед звітом у лупі + bg coding_task (CA-5.3) |
...
| ~~CB4~~ ✅ | ~~Тест-луп не первинна операція~~ → `run_tests`/`run_lint` + no-progress stop-guard + live-fix golden | Закрито — фаза CA-3 повна (CA-3.1…3.5) |
...
| CA-3.5 | Golden trace: навмисно зламаний тест → агент полагодив до green | `tools/tests/golden/` | [x] `golden/fix_loop.json` + `test_fix_loop_golden.py` — hermetic live-fix трейс (run_tests→code_edit→re-run→green) + дзеркальний «застряг→чесний звіт» |

**Вихід CA-3:** «полагодь падіння в `tests/`» → агент ітерує до green або чесно звітує, що застряг.

---

## CA-4 — Plan / multi-file рефактор · ✅ **done**
...
| CA-4.5 | Rename/move рефактор з оновленням імпортів (symbol-граф із CA-2.3) | Golden trace | [x] `rename_symbol` tool — word-boundary (`\bOLD\b`) rg-пошук файлів + транзакційний `code_edit_batch`; оновлює виклики й імпорти; golden у hostagent (User→Account, UserProfile не зачеплено) |
...
| CA-5.2 | Coder→Reviewer→Tester team-pipeline для coding-задач | Reuse `teams.py` | [x] роль `tester` (run_tests/run_lint) + пресет `CODING_ROLES`; coder→code_edit, reviewer→code_review; через наявний `run_team_pipeline` (/teams/spawn roles=…) |
...
| Дата | Версія | Зміна |
|------|--------|-------|
| 2026-06-15 | 2.2 | CA-5.2 done — coding team-pipeline (роль `tester` + пресет `CODING_ROLES`, через `run_team_pipeline`) |
